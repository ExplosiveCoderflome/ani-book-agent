...
六个工具是 `read_project`、`search_project`、`read_skill`、`present_choices`、`propose_patch`、`start_job`。工具只读取、校验或发起运行，不调用模型，也不能绕过 Domain 写权威文件。
...
业务 API 只暴露小说、文件、补丁提案、Job、导出文件和模型设置。错误统一为：
