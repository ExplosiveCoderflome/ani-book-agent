...
- 完成长篇小说的单书串行生产链：从聊天式开书、书级规划推进到章节生产、质量修复与 TXT 导出。
- 重做作者工作台首页与主题系统，并优化聊天式开书：作者说完想法后再整理开书方案，有限候选方向支持一键选择。
- 支持按章节范围逐项确认或自动推进；完善推荐下一步、Markdown 工件编辑、作者保护与下游失效传播。
- 改进运行恢复、活动任务释放、加载反馈和 Mastra Studio 可观测性，开发服务中断后可以安全继续。
