...
export function Conversation({ novelId, initialMessages, initialDraft = "", centered = false, job, externalActivity, appliedProposalIds = [], onProposal, onChange }: { novelId: string; initialMessages: MastraDBMessage[]; initialDraft?: string; centered?: boolean; job?: ProductionJob; externalActivity?: AgentActivity; appliedProposalIds?: string[]; onProposal: (proposal?: PatchProposal) => void; onChange: () => void | Promise<void> }) {
  ...
  useEffect(() => { if (initialDraft) setValue((current) => current || initialDraft); }, [initialDraft]);
