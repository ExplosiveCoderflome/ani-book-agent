...
import { BookOpen, Check, ChevronRight, CircleAlert, Download, Feather, Lightbulb, LoaderCircle, Menu, PanelRightClose, Pencil, Plus, RefreshCw, Settings2, Sparkles, X } from "lucide-react";
...
function HomePage({ onSettings }: { onSettings: () => void }) {
  ...
  return <div className="home-shell"><header className="home-topbar"><div className="brand"><span className="brand-mark"><Feather size={20} /></span><span>ANI 小说 Agent</span></div><button className="quiet-button" onClick={onSettings}><Settings2 size={18} />模型设置</button></header><main className="home-page">
    <section className="hero"><span className="eyebrow">你的长期创作搭档</span><h1>从一次对话，开始写完一本小说。</h1><p>聊想法、做决定、审阅工件。Agent 会记住这部作品的对话，而最终创作事实始终由你批准。</p></section>
    <section className="create-card"><div><h2>开始一部新作品</h2><p>没有想法、没有标题都没关系，创建后让 Agent 陪你找方向。</p></div><form onSubmit={handleSubmit((value) => create.mutate(value))}><input {...register("title", { maxLength: 80 })} placeholder="暂定标题（可以不填）" autoFocus /><select {...register("approvalMode")} aria-label="推进方式"><option value="milestone_approval">里程碑由我批准</option><option value="auto">普通里程碑自动推进</option></select><button className="primary-button" disabled={create.isPending}><Plus />先创建再慢慢想</button></form><ErrorNotice error={create.error} /></section>
    <section className="recent-section"><div className="section-heading"><div><span className="eyebrow">你的书架</span><h2>最近作品</h2></div><span>{bootstrap.data?.novels.length ?? 0} 部</span></div>
      {bootstrap.isLoading ? <div className="loading"><LoaderCircle className="spin" />正在整理书架…</div> : bootstrap.data?.novels.length ? <div className="novel-grid">{bootstrap.data.novels.map((novel) => <Link className="novel-card" key={novel.id} to={`/novels/${novel.id}`}><div className="book-icon"><BookOpen /></div><div><h3>{novel.title}</h3><p>回到主对话</p></div><ChevronRight /></Link>)}</div> : <div className="empty-state">还没有作品。创建第一部小说后，它会出现在这里。</div>}<ErrorNotice error={bootstrap.error} />
    </section>
  </main></div>;
}
...
function DiscoveryCard({ novelId, messageCount, isRunning, onSend, onSaved }: { novelId: string; messageCount: number; isRunning: boolean; onSend: (text: string) => Promise<void>; onSaved: () => void }) {
  const proposal = useMutation({ mutationFn: () => api.proposePreset(novelId) });
  if (proposal.data) return <PresetProposalCard novelId={novelId} proposal={proposal.data} onSaved={onSaved} onReset={() => proposal.reset()} />;
  return <section className="flow-card discovery-card"><div className="card-speaker"><Lightbulb size={16} />聊天式开书</div>{messageCount === 0 ? <><h2>我们先聊，不用先填任何设置</h2><p>选择最接近你现在状态的一句话，或者直接在下面输入。之后我每次只问一个问题，并给你几个备选方案。</p><div className="starter-options">{starterMessages.map(([label, message]) => <button className="secondary-button" key={label} disabled={isRunning} onClick={() => onSend(message)}>{label}<ChevronRight size={17} /></button>)}</div></> : <><h2>继续聊，直到方向让你有感觉</h2><p>你可以选方案、混合两个方案、让 Agent 推荐，或者随时修改主意。觉得已经足够清楚时，再把讨论整理成预设提案。</p><button className="primary-button" disabled={isRunning || proposal.isPending} onClick={() => proposal.mutate()}>{proposal.isPending ? <LoaderCircle className="spin" /> : <Check />}整理当前讨论为开书预设</button></>}<ErrorNotice error={proposal.error} /></section>;
}
...
function BriefProposal({ run, onUpdated, onCanceled }: { run: RunView; onUpdated: () => void; onCanceled: () => void }) {
  ...
  const { register, handleSubmit } = useForm<BriefForm>({ resolver: zodResolver(briefFormSchema), defaultValues: { ...proposal, risksText: proposal.risks.join("\n") } });
  ...
  return <section className="flow-card brief-card"><div className="card-speaker"><Sparkles size={16} />创作搭档 · 等待你的决定</div><h2>小说简报提案</h2><p>这是提案，不是定稿。你可以直接修改任意内容，再批准保存。</p><form className="brief-form" onSubmit={handleSubmit((value) => review.mutate({ action: "approve", brief: toBrief(value) }))}>{briefFields.map(([name, label]) => <label key={name}>{label}<textarea rows={name === "workingTitle" ? 1 : 3} {...register(name)} /></label>)}<label>风险与提醒<textarea rows={4} {...register("risksText")} /></label><div className="approval-box"><strong>批准后会发生什么？</strong><p>当前内容会写入小说简报 Markdown，并成为后续“故事圣经”的上游依据。</p></div><div className="review-actions"><button className="primary-button" type="submit" disabled={review.isPending}><Check />批准并保存</button><button className="secondary-button" type="button" disabled={!feedback.trim() || review.isPending} onClick={() => review.mutate({ action: "revise", feedback })}><RefreshCw />要求调整</button></div><label>给 AI 的调整意见<textarea value={feedback} onChange={(event) => setFeedback(event.target.value)} placeholder="例如：主角目标还不够迫切，希望开篇三章内出现第一次明确胜利。" rows={3} /></label><button type="button" className="text-button danger" onClick={() => review.mutate({ action: "cancel" })}>取消本次生成</button><ErrorNotice error={review.error} /></form></section>;
}

const workflowLabels: Record<WorkflowId, string> = { "novel-brief": "小说简报", "story-bible": "故事圣经", "world-bible": "世界圣经", "character-cast": "角色阵容", "volume-strategy": "卷战略", "volume-outline": "当前卷骨架", "chapter-planning": "章节计划", "quality-repair": "质量修复", "chapter-production": "章节正文生产", "chapter-range": "章节范围", "novel-export": "TXT 导出" };
...
function ArtifactProposalCard({ run, onUpdated, onCanceled }: { run: RunView; onUpdated: () => void; onCanceled: () => void }) {
  ...
  return <section className="flow-card brief-card"><div className="card-speaker"><Sparkles size={16} />创作搭档 · 等待你的决定</div><h2>{proposal.title}</h2><p>这是可编辑提案。批准后才会写入权威工件。</p><textarea className="artifact-editor" rows={18} value={content} onChange={(event) => setContent(event.target.value)} /><div className="review-actions"><button className="primary-button" disabled={review.isPending} onClick={() => review.mutate({ action: "approve", proposal: edited })}><Check />批准并保存</button><button className="secondary-button" disabled={!feedback.trim() || review.isPending} onClick={() => review.mutate({ action: "revise", feedback, proposal: edited })}><RefreshCw />要求调整</button></div><label>调整意见<textarea rows={3} value={feedback} onChange={(event) => setFeedback(event.target.value)} /></label><button className="text-button danger" onClick={() => review.mutate({ action: "cancel" })}>取消本次生成</button><ErrorNotice error={review.error} /></section>;
}

function NextStepCard({ next, pending, onStart, onRange }: { next: NextAction; pending: boolean; onStart: (workflowId: WorkflowId, target?: string) => void; onRange: (start: number, end: number) => void }) {
  const [rangeEnd, setRangeEnd] = useState(next.type === "approve_chapter_range" ? next.chapter : 1);
  if (next.type === "approve_chapter_range") return <section className="flow-card action-card"><div className="card-speaker"><BookOpen size={16} />章节生产授权</div><h2>批准下一段章节范围</h2><p>{next.reason} 章节会严格串行：当前章定稿并回灌连续性后才进入下一章。</p><label>生产到第几章<input type="number" min={next.chapter} max={next.chapter + 99} value={rangeEnd} onChange={(event) => setRangeEnd(Number(event.target.value))} /></label><button className="primary-button" disabled={pending || rangeEnd < next.chapter} onClick={() => onRange(next.chapter, rangeEnd)}><Check />批准第 {next.chapter}–{rangeEnd} 章</button></section>;
  ...
  return <section className="flow-card action-card"><div className="card-speaker"><Sparkles size={16} />推荐下一步</div><h2>{workflowLabels[workflowId]}</h2><p>{next.reason}</p><div className="approval-box"><strong>执行后会发生什么？</strong><p>Agent 会读取权威上游，由 Mastra Workflow 生成、校验并在需要时等待你批准；受保护内容不会被覆盖。</p></div><button className="primary-button" disabled={pending} onClick={() => onStart(workflowId, chapterTarget)}>{pending ? <LoaderCircle className="spin" /> : <Sparkles />}开始{workflowLabels[workflowId]}</button></section>;
}
...
function NovelPage({ onSettings }: { onSettings: () => void }) {
  const { id = "" } = useParams();
  const queryClient = useQueryClient();
  const bootstrap = useQuery({ queryKey: ["bootstrap"], queryFn: api.bootstrap });
  ...
  const range = useMutation({ mutationFn: ({ start, end }: { start: number; end: number }) => api.chapterRange(id, start, end), onSuccess: async (value) => { setRunId(value.runId); localStorage.setItem(storageKey, value.runId); await queryClient.invalidateQueries({ queryKey: ["novel", id] }); } });
  ...
  const flow = (sendMessage: (text: string) => Promise<void>, isRunning: boolean, messageCount: number) => !novel.data?.novel.openingChoices
    ? <DiscoveryCard novelId={id} messageCount={messageCount} isRunning={isRunning} onSend={sendMessage} onSaved={saveChoices} />
    : currentRun?.status === "awaiting_review" && currentRun.proposal ? <BriefProposal key={`${runId}-${currentRun.proposal.openingHook}`} run={currentRun} onUpdated={refresh} onCanceled={resetRun} />
      : currentRun?.status === "awaiting_review" && currentRun.artifactProposal ? <ArtifactProposalCard key={`${runId}-${currentRun.artifactProposal.content.length}`} run={currentRun} onUpdated={refresh} onCanceled={resetRun} />
          ...
          : currentRun?.status === "failed" || currentRun?.status === "canceled" ? <RecoveryCard error={currentRun.error?.message} onReset={resetRun} />
            : novel.data?.nextAction ? <NextStepCard next={novel.data.nextAction} pending={start.isPending || range.isPending} onStart={(workflowId, target) => start.mutate({ workflowId, target })} onRange={(startChapter, endChapter) => range.mutate({ start: startChapter, end: endChapter })} /> : null;

  return <div className={artifactsOpen ? "novel-layout" : "novel-layout panel-closed"}><NovelSidebar currentId={id} onSettings={onSettings} /><main className="conversation-column"><header className="conversation-header"><Link to="/" className="mobile-menu" aria-label="返回书架"><Menu size={20} /></Link><div><small>当前作品</small><h1>{novel.data?.novel.title ?? "正在打开…"}</h1></div><div className="header-actions"><span className="agent-status"><i />Agent 已就绪</span>{!artifactsOpen && <button className="quiet-button" onClick={() => setArtifactsOpen(true)}>查看工件</button>}</div></header>
    {novel.isLoading || chat.isLoading ? <div className="loading conversation-loading"><LoaderCircle className="spin" />正在恢复这部作品的对话…</div> : chat.data ? <Conversation novelId={id} initialMessages={chat.data.messages}>{({ sendMessage, isRunning, messageCount }) => <>{flow(sendMessage, isRunning, messageCount)}<ErrorNotice error={novel.error ?? run.error ?? start.error ?? range.error ?? exportRun.error} /></>}</Conversation> : <div className="loading conversation-loading"><ErrorNotice error={chat.error ?? novel.error} /></div>}
...
export function App() {
  ...
  return <><Routes><Route path="/" element={<HomePage onSettings={() => setSettingsOpen(true)} />} /><Route path="/novels/:id" element={<NovelPage onSettings={() => setSettingsOpen(true)} />} /></Routes><ModelSetup open={settingsOpen || required} required={required} onClose={() => setSettingsOpen(false)} /></>;
}
