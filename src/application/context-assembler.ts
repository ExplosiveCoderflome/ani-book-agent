...
export const CONTEXT_CONTENT_BUDGET = 8_000;
export const CREATOR_GUIDELINES_PATH = "CREATOR.md";
export const CREATOR_GUIDELINES_BUDGET = 4_000;
export const SELECTED_ASSET_CONTEXT_BUDGET = 12_000;
...
export async function assembleNovelContext(repository: NovelRepository, novelId: string, keys: string[]) {
  ...
  const [creatorGuidelines, items] = await Promise.all([readCreatorGuidelines(repository, novelId), Promise.all(keys.map(async (key) => {
  ...
  return renderContext(state.title, state.openingChoices, items, creatorGuidelines);
}
...
export async function assembleAvailableNovelContext(repository: NovelRepository, novelId: string, keys: string[]) {
  ...
  const [creatorGuidelines, items] = await Promise.all([
    readCreatorGuidelines(repository, novelId),
    Promise.all(ready.map(async (key) => ({ key, content: (await repository.readArtifact(novelId, key)).content }))),
  ]);
  const missing = keys.filter((key) => !ready.includes(key));
  return `${missing.length ? `声明依赖中尚未就绪的工件（不得当作已完成）：${missing.join(", ")}` : "声明依赖均已就绪。"}\n\n${renderContext(state.title, state.openingChoices, items, creatorGuidelines)}`;
}

export async function readCreatorGuidelines(repository: NovelRepository, novelId: string) {
  const file = await repository.readWorkspaceFile(novelId, CREATOR_GUIDELINES_PATH).catch(() => undefined);
  return file?.content.trim().slice(0, CREATOR_GUIDELINES_BUDGET) ?? "";
}

/** Resolve the one file or artifact the author explicitly put into the chat context. */
export async function readSelectedNovelContext(repository: NovelRepository, novelId: string, selection: { artifactKey?: string; filePath?: string }) {
  if (selection.artifactKey) {
    const artifact = await repository.readArtifact(novelId, selection.artifactKey).catch(() => undefined);
    if (artifact) return { label: `工件 ${selection.artifactKey}`, content: artifact.content.slice(0, SELECTED_ASSET_CONTEXT_BUDGET), truncated: artifact.content.length > SELECTED_ASSET_CONTEXT_BUDGET };
  }
  if (selection.filePath) {
    const file = await repository.readNovelFile(novelId, selection.filePath).catch(() => undefined);
    if (file) return { label: `文件 ${file.path}`, content: file.content.slice(0, SELECTED_ASSET_CONTEXT_BUDGET), truncated: file.content.length > SELECTED_ASSET_CONTEXT_BUDGET };
  }
  return undefined;
}

function renderContext(title: string, openingChoices: NovelState["openingChoices"], items: Array<{ key: string; content: string }>, creatorGuidelines = "") {
  const sections = [`作品：${title}`, `开书选择：${JSON.stringify(openingChoices ?? {})}`, ...(creatorGuidelines ? [`\n## 作者创作约束（workspace/${CREATOR_GUIDELINES_PATH}）\n${creatorGuidelines}`] : []), "以下为按任务优先级压缩的权威上下文；每项均来自声明依赖。"];
